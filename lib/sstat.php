...
class SStats { 
	...
	public static function getCV19Monitor($prov){
		...
		$stmt = $mysqli->prepare('SELECT data, totale_casi FROM covid19_province WHERE codice_provincia = ? ORDER BY DATA');
	...
	public static function getCV19Table($prov){
		$mysqli = new mysqli(GlobalConfig::$dbconnect['host'], GlobalConfig::$dbconnect['user'], GlobalConfig::$dbconnect['pswd'], GlobalConfig::$dbconnect['name']);
		...
		$stmt = $mysqli->prepare('SELECT codice_regione, data, incremento, totale_casi, ROUND(CASE WHEN ROW_NUMBER() OVER (PARTITION BY codice_provincia ORDER BY data DESC) THEN SUM(incremento) OVER (PARTITION BY codice_provincia ORDER BY data ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) END / 3) AS dayincr FROM (SELECT t2.codice_regione, t2.codice_provincia, t2.data, t2.totale_casi, GREATEST(0,t2.totale_casi - t1.totale_casi) AS incremento FROM covid19_province t1 INNER JOIN covid19_province t2 ON CAST(t1.data AS DATETIME) =  DATE_ADD(CAST(t2.data AS DATETIME), INTERVAL -1 DAY)  AND t1.codice_provincia = t2.codice_provincia WHERE t1.codice_provincia = ?) A ORDER BY data DESC');
		$stmt->bind_param('s', $prov);
		$stmt->execute();
	...
	public static function getCV193Day($prov){
		$mysqli = new mysqli(GlobalConfig::$dbconnect['host'], GlobalConfig::$dbconnect['user'], GlobalConfig::$dbconnect['pswd'], GlobalConfig::$dbconnect['name']);
	...
	public static function getCV19Increment($prov){
		$mysqli = new mysqli(GlobalConfig::$dbconnect['host'], GlobalConfig::$dbconnect['user'], GlobalConfig::$dbconnect['pswd'], GlobalConfig::$dbconnect['name']);
		...
		$stmt = $mysqli->prepare('SELECT t2.data, GREATEST(0,t2.totale_casi - t1.totale_casi) AS incremento FROM covid19_province t1 INNER JOIN covid19_province t2 ON CAST(t1.data AS DATETIME) =  DATE_ADD(CAST(t2.data AS DATETIME), INTERVAL -1 DAY)  AND t1.codice_provincia = t2.codice_provincia WHERE t1.codice_provincia = ?');
		$stmt->bind_param('s', $prov);
		$stmt->execute();
